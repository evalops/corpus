...
fn occ(host: &str, seq: i64, path: &str, size: i64, observed: chrono::DateTime<chrono::Utc>) -> OccurrenceInfo {
    OccurrenceInfo {
        host_name: host.into(),
...
async fn analyst_surface_end_to_end() {
    ...
    tenant::ensure_tenant(&pool, tenant_id, &slug, "Analyst test").await.unwrap();

    let t0 = chrono::DateTime::parse_from_rfc3339("2024-06-01T10:00:00Z").unwrap().with_timezone(&chrono::Utc);
    ...
    let (seed, sha_seed) = commit(&pool, &cas, tenant_id, occ("host1", 1, "/tmp/bad.exe", 10, t0), b"bad seed CORPUS_ANALYST_MARKER").await;
    let (cand, sha_cand) = commit(&pool, &cas, tenant_id, occ("host1", 2, "/tmp/helper.dll", 11, t0 + h), b"helper dll bytes").await;
    let (_, sha_common) = commit(&pool, &cas, tenant_id, occ("host1", 3, "/bin/common", 12, t0 + h), b"common bytes everywhere").await;
    for (i, host) in ["host2", "host3", "host4"].iter().enumerate() {
        commit(&pool, &cas, tenant_id, occ(host, i as i64 + 4, "/bin/common", 12, t0 + h), b"common bytes everywhere").await;
    }
    commit(&pool, &cas, tenant_id, occ("host1", 7, "/opt/old.so", 9, t0 - chrono::Duration::days(2)), b"old library bytes").await;

    // ===== 1. PREVALENCE =====
    let p_common = analyst::prevalence_for(&pool, tenant_id, {
        let row: Option<(Uuid,)> = sqlx::query_as("SELECT id FROM artifact WHERE tenant_id = $1 AND sha256 = decode($2,'hex')")
            .bind(tenant_id)
            .bind(&sha_common)
            .fetch_optional(&pool)
            .await
    ...
    let p_seed = analyst::prevalence_for(&pool, tenant_id, seed).await.unwrap();
    assert_eq!(p_seed.host_count, 1);
    assert_eq!(p_seed.first_observed, Some(t0));

    // ===== 2. rarity search: only the two host_count<=1 artifacts =====
    let hits = analyst::rarity_search(&pool, tenant_id, 1, t0 - chrono::Duration::days(3), None, 50)
        .await
        .unwrap();
    ...
    assert!(!shas.contains(&sha_common.as_str()), "4-host artifact is not rare");

    // ===== 3. opinions + audit =====
    opinions::set_opinion(&pool, tenant_id, seed, "suspicious", "analyst-1", "odd imports").await.unwrap();
    opinions::set_opinion(&pool, tenant_id, seed, "malicious", "analyst-1", "confirmed C2").await.unwrap();
    let current = opinions::current_opinion(&pool, tenant_id, seed).await.unwrap().unwrap();
    assert_eq!(current.opinion, "malicious");
    let history = opinions::opinion_history(&pool, tenant_id, seed).await.unwrap();
    assert_eq!(history.len(), 2);
    assert!(history[0].superseded_by.is_some(), "first opinion superseded");
    ...
    let (row, _) = triggers::create_trigger(&pool, tenant_id, "match-hook", triggers::CONDITION_HUNT_MATCH, &hook.base, Some(secret.clone()))
        .await
        .unwrap();
    assert!(row.enabled);

    // verdict trigger too, to assert opinion-set firing
    triggers::create_trigger(&pool, tenant_id, "verdict-hook", triggers::CONDITION_MALICIOUS_VERDICT, &hook.base, Some(secret.clone()))
        .await
        .unwrap();
    opinions::set_opinion(&pool, tenant_id, cand, "suspicious", "analyst-2", "near bad seed").await.unwrap();
    ...
    .await
    .unwrap();
    assert_eq!(outbox_verdict2, 1, "suspicious opinion fires verdict trigger");
    ...
    .await
    .unwrap();
    let bundle = corpus_core::registry::publish_bundle(&pool, tenant_id, &[rule.id], false).await.unwrap();
    ...
    assert!(events.iter().all(|(_, valid)| *valid), "every delivery carries a valid HMAC signature");
    let types: Vec<&str> = events.iter().filter_map(|(e, _)| e["type"].as_str()).collect();
    assert!(types.contains(&"hunt_match"), "hunt match event delivered: {types:?}");
    assert!(types.contains(&"malicious_verdict"), "verdict event delivered: {types:?}");

    // ===== 5. dropper hunt =====
    let droppers = analyst::dropper_candidates(&pool, tenant_id, &sha_seed, 3, 24, 50).await.unwrap();
    let dshas: Vec<&str> = droppers.iter().map(|d| d.sha256.as_str()).collect();
    assert!(dshas.contains(&sha_cand.as_str()), "near-in-time rare file is a candidate");
    assert!(!dshas.contains(&sha_common.as_str()), "common file excluded (prevalence)");
    let old_sha = hash::sha256_hex(b"old library bytes");
    assert!(!dshas.contains(&old_sha.as_str()), "out-of-window file excluded");
    ...
    // ===== 6. proof of absence =====
    let br = report::by_sha256(&pool, tenant_id, &"00".repeat(32), false).await.unwrap();
    ...
    assert_eq!(att.artifacts_evaluated, 4, "all four endpoint artifacts evaluated");
    ...
    .await
    .unwrap();
    let miss_bundle = corpus_core::registry::publish_bundle(&pool, tenant_id, &[miss_rule.id], false).await.unwrap();
    let miss_hunt = corpus_core::hunts::create_hunt(&pool, tenant_id, &miss_bundle.digest).await.unwrap();
    let miss_hunt = corpus_core::hunts::run_hunt(&pool, &cas, tenant_id, miss_hunt.id).await.unwrap();
    assert_eq!(miss_hunt.matched, 0);
    let br2 = report::by_hunt(&pool, tenant_id, miss_hunt.id, false).await.unwrap();
    ...
    let _intel_fin = ingest::finalize(
        &pool,
        &cas,
        tenant_id,
        &{
            ...
            ingest::stage_upload(&pool, &cas, tenant_id, upload_id, b"intel scoped bytes").await.unwrap();
            FinalizeRequest {
                upload_id,
                sha256: sha,
                size_bytes: 18,
    ...
    .await
    .unwrap();
    let hits2 = analyst::rarity_search(&pool, tenant_id, 10, t0 - chrono::Duration::days(3), None, 50)
        .await
        .unwrap();
    assert!(
        hits2.iter().all(|h| h.sha256 != hash::sha256_hex(b"intel scoped bytes")),
