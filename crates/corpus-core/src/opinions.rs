...
pub async fn set_opinion(
    ...
    if !OPINIONS.contains(&opinion.as_str()) {
        return Err(Error::BadRequest(format!(
            "invalid opinion {opinion:?}; supported: {}",
            OPINIONS.join(", ")
        )));
    }
    let id = Uuid::new_v4();
    ...
    if let Some((prev,)) = previous {
        sqlx::query(
            "UPDATE artifact_opinion SET superseded_by = $3 WHERE tenant_id = $1 AND id = $2",
        )
        .bind(tenant)
        .bind(prev)
        .bind(id)
...
pub async fn current_opinion(
    pool: &PgPool,
    tenant: Uuid,
    artifact: Uuid,
) -> Result<Option<OpinionRow>> {
...
pub async fn opinion_history(
    pool: &PgPool,
    tenant: Uuid,
    artifact: Uuid,
) -> Result<Vec<OpinionRow>> {
