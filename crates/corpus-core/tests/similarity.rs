...
async fn similarity_pipeline_end_to_end() {
    ...
    tenant::ensure_tenant(&pool, tenant_id, &slug, "Similarity test")
        .await
        .unwrap();
    ...
    let sim_a = edges::similar_view(&pool, tenant_id, &sha_a)
        .await
        .unwrap()
        .unwrap();
    let norm = edge_between(&sim_a.edges, &sha_b, edge_type::NORMALIZED_EQUIVALENT)
        .expect("A<->B normalized edge");
    assert_eq!(
        norm.model_version,
        corpus_core::similarity::model::MODEL_VERSION
    );
    assert_eq!(norm.evidence["matched_feature"], "imphash");
    assert!(
        edge_between(&sim_a.edges, &sha_c, edge_type::NORMALIZED_EQUIVALENT).is_none(),
        "C has different imports"
    );

    // --- variant groups: A and B together, C alone
    let var_a = edges::variants_view(&pool, tenant_id, &sha_a)
        .await
        .unwrap()
        .unwrap();
    ...
    let var_c = edges::variants_view(&pool, tenant_id, &sha_c)
        .await
        .unwrap()
        .unwrap();
    ...
    let sim_d = edges::similar_view(&pool, tenant_id, &sha_d)
        .await
        .unwrap()
        .unwrap();
    let weak = edge_between(&sim_d.edges, &sha_e, edge_type::BYTE_SIMILAR)
        .expect("D<->E byte_similar edge");
    assert!(
        weak.score >= 40.0,
        "near-identical blobs score high, got {}",
        weak.score
    );
    assert!(weak.evidence["note"]
        .as_str()
        .unwrap()
        .contains("never merges"));
    let var_d = edges::variants_view(&pool, tenant_id, &sha_d)
        .await
        .unwrap()
        .unwrap();
    assert!(
        var_d.members.is_empty(),
        "fuzzy alone never merges groups (28.5)"
    );
    ...
    let count_before: i64 =
        sqlx::query_scalar("SELECT count(*) FROM similarity_edge WHERE tenant_id = $1")
            ...
            .await
            .unwrap();
    let _ = edges::analyze_artifact(&pool, tenant_id, a, "pe", &pe_a)
        .await
        .unwrap();
    let _ = edges::analyze_artifact(&pool, tenant_id, b, "pe", &pe_b)
        .await
        .unwrap();
    let var_a2 = edges::variants_view(&pool, tenant_id, &sha_a)
        .await
        .unwrap()
        .unwrap();
    ...
    let br = report::by_sha256(&pool, tenant_id, &sha_a, true)
        .await
        .unwrap();
    ...
    assert!(
        expansion
            .group_occurrences
            .iter()
            .any(|o| o.artifact_sha256 == sha_b),
        "expansion includes B's occurrences"
    );

    // Weak neighbors labeled as leads, not members.
    let br_d = report::by_sha256(&pool, tenant_id, &sha_d, true)
        .await
        .unwrap();
    let exp_d = br_d.variant_expansion.unwrap();
    assert!(
        exp_d.group_artifacts.is_empty(),
        "weak edges do not expand groups"
    );
    let lead = exp_d
        .weak_leads
        .iter()
        .find(|l| l.other_sha256 == sha_e)
        .expect("weak lead E");
    assert_eq!(lead.edge_type, edge_type::BYTE_SIMILAR);

    // Without the flag there is no expansion.
    let br_plain = report::by_sha256(&pool, tenant_id, &sha_a, false)
        .await
        .unwrap();
    ...
    tenant::ensure_tenant(&pool, other, &oslug, "Other")
        .await
        .unwrap();
    assert!(edges::similar_view(&pool, other, &sha_a)
        .await
        .unwrap()
        .is_none());
    let occ_count: i64 =
